  ...
  implements SecurityProfileRepository
{
    ...
    const organization = existing
      ...
      : await this.client.organization.create({
          data: {
            ...
            dataTypes: { create: this.organizationDataTypes(input.dataHandling) },
  ...
  async createVendor(input: VendorInput): Promise<Vendor> {
    ...
    const vendor = await this.client.vendor.create({
      data: {
        ...
        dataTypes: {
          create: input.dataProcessed.map((name) => ({
            organizationDataType: this.connectOrCreateDataType(
              organization.id,
              name
            ),
          })),
  ...
  private connectOrCreateDataType(organizationId: string, name: string) {
    return {
      connectOrCreate: {
        where: {
          organizationId_name: {
